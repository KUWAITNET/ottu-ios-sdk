...
@objc public protocol TapApplePayButtonDataSource {
    ...
    var tapApplePayRequest:ApplePayRequest { get }
}
...
@objc public protocol TapApplePayButtonDelegate {
     ...
     */
    func tapApplePayFinished(payment: PKPayment, completion: @escaping (PKPaymentAuthorizationResult) -> Void)
...
@objcMembers public class TapApplePayButton: UIView {
    ...
    internal let tapApplePay = ApplePay()
    ...
    internal func startApplePaymentAuthorization() {
        ...
        if let nonNullDataSource = dataSource {
            ...
            tapApplePay.authorizePayment(in: self.findViewController()!, for: nonNullDataSource.tapApplePayRequest, tokenized: {
                [weak self] (payment, completion) in
                if let nonNullDelegate = self?.delegate {
                    ...
                    nonNullDelegate.tapApplePayFinished(payment: payment, completion: completion)
                }
            }, dissmissed: {
                if let nonNullDelegate = self.delegate {
                    // If there is alistener, let him know that the authorization is done with the provided tokem
                    nonNullDelegate.tapApplePayDissmised()
                }
            })
        } else {
